...
def _build_transitions() -> list[Transition]:
  ...
  return [
    ...
    Transition([None],                                      State.TURNING_ARROW,  lambda x: x.arrow is not None),
    Transition([None],                                      State.TURNING,        lambda x: abs(x.turning - 0.5) >= config.TURNING_TRIGGER_OFFSET),
